...
class SystemDesign:
    ...
    '''
    Provides functionality to optimize dam systems by evolutionary computing framework
    ...
    def __init__(
        ...
        '''
        Initializes the class instance with a default attribute ``lifespan_epsilon`` to :math:`10^{-6}`.
    ...
    def _validate_variable_config(
        ...
        '''
        A private function that validates the input configuration of dams and their storage volumes.
        '''
    ...
    def _validate_model_config(
        ...
        for m_key, m_val in model_config.items():
            if not isinstance(m_key, str):
                raise TypeError(
                    f'Key "{m_key}" in model_config must be a string, but got type "{type(m_key).__name__}"'
                )
            if m_key not in vars_valid:
                raise NameError(
                    f'Invalid variable "{m_key}" in model_config; valid names are {vars_valid}'
                )
        ...
        for r_key in required_keys:
            if r_key not in model_config:
                raise KeyError(
                    f'Required key "{r_key}" is missing from model_config'
                )
    ...
    def _validate_algorithm_config(
        ...
        '''
        A private function that validates the genetic algorithm configuration.
        '''
    ...
    def _validate_objectives(
        ...
        '''
        A private method that validates the list of objectives.
        '''
    ...
    def _validate_constraints(
        ...
        '''
        A private method that validates the constratint dictionary.
        '''
    ...
    def _df_from_solutions(
        ...
        stream_file: str,
        model_config: dict[str, typing.Any],
        solution_sorting: typing.Optional[str]
    ) -> pandas.DataFrame:

        '''
        A private method that constructs a DataFrame from the non-dominated solutions
        obtained from the method :meth:`OptiDamTool.SystemDesign.sedimentation_management_by_ga`.
        '''
        ...
        for obj in objectives:
            if obj == 'lifespan':
                ...
                ub_val = dam_number * numpy.log(model_config['year_limit'])
            if obj == 'lifespan_std':
                lb_val = 0
                ub_val = model_config['year_limit'] / 2
            if obj == 'storage_sum':
        ...
        for idx, sol in enumerate(solutions):
            ...
            sol_output = Network().storage_dynamics_lite(
        ...
        # DataFrame sorting
        if solution_sorting is not None:
            # by objective directions
            if solution_sorting == 'by_objective_directions':
                ascending = []
                for obj in objectives:
                    obj_dir = True if objs_dirs[obj] == 'min' else False
            ...
            # by stream identifiers of dams
            if solution_sorting == 'by_dam_idenfiers':
                df = df.sort_values(
                    by=col_dam,
                    ignore_index=True
                )
            # by Euclidean distance from normalized solutions to the ideal solution
            if solution_sorting == 'by_metric_euclidean':
                df = df.sort_values(
                    by=col_euclidean,
                    ignore_index=True
                )
    ...
    def compute_storage_variability(
        ...
        df : DataFrame
            ...
            generated by :meth:`OptiDamTool.Network.storage_dynamics_lite`.
    ...
    def scenario_sedimentation_management(
        self,
        ...
        '''
        Generates objective and constraint values for the scenario produced by
        :meth:`OptiDamTool.SystemDesign.solution_sedimentation_management_by_ga`.
        ...
        model_config : dict
            Dictionary specifying input variable configurations for :meth:`OptiDamTool.Network.storage_dynamics_lite`.
        ...
        # storage dynamics dictionary
        model_output = Network().storage_dynamics_lite(
            stream_file=stream_file,
    ...
    def mapping_constraint_operator(
        ...
        '''
        Provides a dictionary mapping optimization constraints
    ...
    def mapping_objective_direction(
        ...
        '''
        Provides a dictionary mapping optimization objectives to their respective directions.
    ...
    def solution_sedimentation_management_by_ga(
        self,
        dam_number: int,
        storage_bounds: tuple[int, int],
        ...
        solution_sorting: typing.Optional[str] = None
    ) -> pandas.DataFrame:

        '''
        Optimizes dam locations and storage volumes using a multi-objective genetic algorithm,
        ...
        solution_sorting : str, optional
            Method to sort the output DataFrame of non-dominated solutions. Default is None, which means no sorting is applied.

            - ``by_objective_directions``
                Sort the DataFrame by the ``<obj>(<dir>)`` columns according to the directions
                of the specified ``objectives`` (``min`` and ``max`` indicate ascending and descending orders, respectively).

            - ``by_dam_identifiers``
                Sort the DataFrame by the ``d_<i>`` columns in ascending order.

            - ``by_euclidean_metric``
                Sorts the DataFrame by the ``Euclidean`` distance of normalized solutions
                to the ideal solution, in ascending order.
        ...
        # check validity of input variables type
        var_type = typing.get_type_hints(
            obj=self.solution_sedimentation_management_by_ga
        )
        ...
        # check validity of solution sorting methods
        if solution_sorting is not None:
            sorting_methods = [
                'by_objective_directions',
                'by_dam_idenfiers',
            ...
            if solution_sorting not in sorting_methods:
                raise ValueError(
                    f'Invalid solution_sorting name "{solution_sorting}"; valid names are {sorting_methods}'
                )

        # check validity of variable configuration for optimization
        stream_ids = self._validate_variable_config(
            stream_file=stream_file,
        ...
        # check validity of model condiguration
        ...
        problem.function = functools.partial(
            self.scenario_sedimentation_management,
            storage_multiplier=storage_multiplier,
            stream_file=stream_file,
            model_config=model_config,
            objectives=objectives,
        ...
        if processes is None:
            ...
            cpu_max = processes
        batch_number = seeds // cpu_max if seeds % cpu_max == 0 else seeds // cpu_max + 1

        # time start for simulating the problem by multiple experiment
        experiment_start = time.time()

        # run algorithm with parallel computing support
        with platypus.ProcessPoolEvaluator(processes=cpu_max) as evaluator:
        ...
        solution_df = self._df_from_solutions(
            ...
            stream_file=stream_file,
            model_config=model_config,
            solution_sorting=solution_sorting
        )
